...
export const Route = createFileRoute('/webgl/multiple-rectangles')({
  component: MultipleRectangles,
});

function MultipleRectangles() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        ...
        const [row_gap, column_gap] = [4, 4];
        const [margin_left, margin_bottom] = [4, 4];
        const [x_offset, y_offset] = [margin_left + i * column_gap, margin_bottom + j * row_gap];

        const [x1, y1] = [x_offset + i * width, y_offset + j * height];
        const [x2, y2] = [x_offset + i * width, y_offset + j * height + height];
        const [x3, y3] = [x_offset + i * width + width, y_offset + j * height + height];
        const [x4, y4] = [x_offset + i * width + width, y_offset + j * height];

        const positions = [x1, y1, x2, y2, x3, y3, x3, y3, x4, y4, x1, y1];
